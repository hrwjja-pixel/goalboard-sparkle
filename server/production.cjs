...
var __dirname;
try {
  __dirname = typeof __dirname !== "undefined" ? __dirname : import_path.default.dirname((0, import_url.fileURLToPath)(""));
} catch {
  __dirname = __dirname;
}
...
app.delete("/api/projects/:id", async (req, res) => {
  try {
    ...
    const childrenCount = await prisma3.project.count({
      where: { parentId: id }
    });
    if (childrenCount > 0) {
      return res.status(400).json({
    ...
    const project = await prisma3.project.findUnique({ where: { id } });
    await prisma3.project.delete({
      where: { id }
    });
    await req.audit?.({
      action: "DELETE",
...
app.post("/api/goals", async (req, res) => {
  try {
    ...
    const progress = goalData.progress !== void 0 ? goalData.progress : 0;
